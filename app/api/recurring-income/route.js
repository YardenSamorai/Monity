...
export async function GET(request) {
  try {
    ...
    if (householdId) {
      ...
      if (member) {
        // Family recurring incomes: must have householdId AND isShared = true
      ...
      // Personal recurring incomes: must have householdId IS NULL AND isShared = false
...
export async function POST(request) {
  try {
    ...
    if (isShared && householdId) {
      ...
      // If user is not a member, verifiedHouseholdId stays null (will create personal income)
    } else if (!isShared) {
    ...
    notifyRecurringIncomeChange(user.id, 'created', recurringIncome).catch((err) => console.error('Pusher error:', err))
    notifyDashboardUpdate(user.id, { action: 'recurring_income_created' }).catch((err) => console.error('Pusher error:', err))
