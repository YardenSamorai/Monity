...
export function RealtimeProvider({ children, userId, householdId }) {
  ...
  useEffect(() => {
    ...
    dashboardChannel.bind(EVENTS.DASHBOARD_UPDATE, (data) => {
      ...
      console.log('[RealtimeContext] Listeners for DASHBOARD_UPDATE:', listenersRef.current.get(EVENTS.DASHBOARD_UPDATE)?.length || 0)
      refreshData()
    ...
    dashboardChannel.bind(EVENTS.CREDIT_CARD_TRANSACTION, (data) => {
      ...
      console.log('[RealtimeContext] Listeners for this event:', listenersRef.current.get(EVENTS.CREDIT_CARD_TRANSACTION)?.length || 0)
      toast.info(
  ...
  const subscribe = useCallback((event, callback) => {
    ...
    console.log('[RealtimeContext] Subscribed to', event, 'total listeners:', listeners.length)
    ...
    return () => {
      ...
      console.log('[RealtimeContext] Unsubscribed from', event)
    }
  }, [])
